...
import { TEventWithRelations } from "@/types/events";
import Image from "next/image";
import Link from "next/link";
import { updateTicketingAvailability } from "../actions";

interface EventDetailProps {
  selectedEvent: TEventWithRelations;
}

function EventDetail(props: EventDetailProps) {
  const { selectedEvent } = props;

  const handleTicketingChange = async (
    ticketingId: string,
    enabled: boolean
  ) => {
    console.log(ticketingId, enabled);
    const response = await updateTicketingAvailability(ticketingId, enabled);
      ...
      <article className="flex md:flex-row flex-col h-full justify-between gap-2 text-black">
        <section className="flex flex-col md:flex-row gap-20">
          <article>
            ...
            <p>Line Up: {selectedEvent.DJsLineUp.join(", ")}</p>
          </article>
          <article>
            <h2 className="text-xl font-bold mb-2">Etapas: </h2>
            <ul>
              {selectedEvent.ticketing.map((ticketing) => (
                <li key={ticketing.id} className="flex gap-2 mb-2">
                  <span className="w-[200px]">{ticketing.name}</span>
                  <SwitchInput
                    id={ticketing.name}
                    defaultChecked={ticketing.isAvailable}
                    onChange={(enabled) => {
                      handleTicketingChange(ticketing.id, enabled);
                    }}
                  />
                </li>
              ))}
